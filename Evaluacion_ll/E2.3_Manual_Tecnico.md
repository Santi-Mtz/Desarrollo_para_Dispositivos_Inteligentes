...
*   `POST /sync/publish`: Envío de eventos de sincronización a la Smart TV. Requiere token JWT en cabecera `Authorization` (**SA.0.B**).

### 4.2 Endpoints de Administración (Cabecera `Authorization: Bearer <token>` de administrador requerida)
...
## 5. Arquitectura del Ecosistema de Dispositivos Inteligentes

Para validar la integración multi-pantalla y la captura de sensores, el sistema se extiende con el siguiente ecosistema:
